...
const exportToExcel = () => {
  ...
  return (
    <div className="py-6 relative">
     ...
     <div className="px-6 fixed w-full top-0 bg-white z-10 shadow-lg h-16 py-3.5">
    ...
    if (e.key === "Enter") handleSubmit(e as any);
       ...
       <div className="flex items-center gap-2">
        <div className="font-bold text-2xl mr-2">AIS</div>
          <Button type="submit" variant={"secondary"} className="border" disabled={loading}><Search/> {loading ? "Đang tải..." : "Tìm kiếm"} 
        <span className="px-2 py-1 rounded-sm border text-xs bg-white">Enter</span></Button>

         <div className="flex gap-2">
          <Label className="pl-2">MMSI:</Label>
          <Input className="min-w-40" value={mmsi} onChange={(e) => setMmsi(e.target.value)} placeholder="Ví dụ: 574792499" />
        </div>
      ...
      {/* Date Picker Begin */}
        <div className="flex gap-2">
          ...
          <Popover >
            <PopoverTrigger asChild>
              <Button variant="outline" className="w-44 justify-start text-left">
                {beginDate ? beginDate.toLocaleDateString() : "Chọn ngày bắt đầu"}
              </Button>
            </PopoverTrigger>
       ...
       <div className="flex gap-2">
          ...
          {/* <Button type="submit" variant={"secondary"} className="border" disabled={loading}><Search/> {loading ? "Đang tải..." : "Tìm kiếm"}</Button> */}
        </div>
       </div>
       ...
       <div className="flex items-center gap-2">
        ...
        {data.length > 0 && (
            <Button className="has-[>svg]:px-4" onClick={exportToExcel}><Download/> <p className="text-sm">Export</p></Button>
            )}
  ...
  <p className="ml-2 text-sm">{loading && "Đang tải dữ liệu..."}</p>
      ...
      {data.length > 0 && (
        <div className="">
            <p className="text-sm text-center my-1 mb-2">(Lưu ý: Hệ thống chỉ hiển thị 1000 bản ghi đầu tiên, tải xuống file xlsx để theo dõi toàn bộ dữ liệu.)</p>
            <Table>
          <TableHeader className="text-[13px] border-t h-10 ">
            <TableRow>
              ...
              <TableHead className="px-4 border-l">Tốc độ (knots)</TableHead>
          ...
          <TableBody className="text-[13px] border-b">
            {data.slice(0,1000).map((item, idx) => (
              <TableRow key={idx}>
                ...
                <TableCell className="px-4 h-10 border-l">{formatTime(item.utcpos)}</TableCell>
                <TableCell className="px-4 h-10 border-l">{`${toDMS(item.lat, true)} ${toDMS(item.lon, false)}`}</TableCell>
                <TableCell className="px-4 h-10 border-l">{item.cog}</TableCell>
                <TableCell className="px-4 h-10 border-l">{item.sog}</TableCell>
                <TableCell className="px-4 h-10 border-l">{formatDistance(item.distance)}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
